    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            function (position) {
				...
				// Si se abrió index con un parametro URL "marker", se centra en el marker en vez del usuario
                ...
                userMarker = L.marker([userLat, userLng], {
                    icon: L.divIcon({
                        ...
                        html: '<div style="background-color:#007bff;color:white;border-radius:50%;width:22px;height:22px;display:flex;align-items:center;justify-content:center;font-size:14px;border:3px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.3);">馃搷</div>',
                        iconSize: [26, 26],
                        iconAnchor: [13, 13]
                    })
                }).addTo(map);
...
function addRealSheltersAndServices() {
    const places = [
        ...
        // 馃惐 脩u帽oa
        {
            nombre: "Centro de Rescate Canino 脩u帽oa",
            direccion: "Av. Vicu帽a Mackenna 1590, 脩u帽oa",
            lat: -33.4636,
            lon: -70.5987,
    ...
    const icon = L.divIcon({
        ...
        html: '<div style="background-color:#28a745;color:white;border-radius:50%;width:34px;height:34px;display:flex;align-items:center;justify-content:center;font-size:18px;border:3px solid white;box-shadow:0 3px 6px rgba(0,0,0,0.4);">馃彔</div>',
        iconSize: [34, 34],
        iconAnchor: [17, 17]
    });

    places.forEach(item => {
        const webLink = item.web ? `<div><a href="${item.web}" target="_blank">馃寪 Sitio web</a></div>` : "";
    ...
    console.log(`鉁?Refugios y servicios cargados: ${places.length}`);
}
