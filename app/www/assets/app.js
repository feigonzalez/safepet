...
// almacena la información del usuario. Se obtiene directamente desde localStorage
var userData={};
...
window.addEventListener("load",()=>{
	...
	// por cada entrada del objeto se帽alado en el atributo.
	...
	// 馃憞 NUEVO: si venimos desde Flow, procesa el resultado del pago
	procesarFlow();
	
	// si la sesión está iniciada, se crea un intervalo para buscar notificaciones nuevas cada 15 segundos
...
async function processContents(self){
	...
	// Maneja elementos que no son <img> pero que muestran im谩genes
	...
	// Usa el valor [data-columns] para determinar el n煤mero de columnas.
	...
	// Hace que el bot贸n #backButton redirija a la p谩gina previa
...
// Cierra el modal.
// No hay ninguna raz贸n para que haya m谩s de un modal a la vez
...
// Si se pasa una funci贸n como closeCallback, esa funci贸n se llamar谩 al cerrar el modal (usando closeModal())
...
// Funci贸n para mostrar opciones de foto
            ...
            <div class="modal">
                <div class="column mh-1">
                    <div class="row ta-center">
                        ...
                        <p class="metaText" style="margin:0.5rem 1rem">驴C贸mo quieres agregar la foto de tu mascota?</p>
                    </div>
...
// Funci贸n para mostrar modal de verificaci贸n
...
// Funci贸n para mostrar modal de espera. 
...
// Funci贸n para mostrar modal de verificaci贸n, versi贸n s铆ncrona
...
// Funci贸n para mostrar modal de confirmaci贸n. Parecido al de verificaci贸n, pero usa funciones de callback en vez de promesas
...
// Mejorar la funci贸n showUploadedImageAsBg para trabajar con el c铆rculo
...
// Funci贸n espec铆fica para escanear QR en registro
async function scanQRForRegistration() {
    try {
        const confirmed = await showVerificationModal(
            'Escanear C贸digo QR',
            '驴Est谩s seguro de que quieres escanear el c贸digo QR? Esto puede completar autom谩ticamente algunos campos del formulario.',
            'Escanear',
            'Cancelar'
        );
        ...
        if (confirmed) {
            ...
            if (qrContent) {
                // Procesar el contenido del QR y llenar campos autom谩ticamente
...
// Funci贸n para procesar contenido QR y llenar formulario
function processQRContent(qrContent) {
    try {
        ...
        alert('Datos del QR cargados autom谩ticamente. Revisa y completa la informaci贸n faltante.');
    } catch (e) {
        // Si no es JSON, mostrar el contenido como texto
        alert(`C贸digo QR detectado: ${qrContent}\n\nPor favor, completa manualmente los datos del formulario.`);
    }
}
...
function locate(callbackSuccess, callbackError = ()=>{}){
	if(!navigator.geolocation){
		console.warn("Este navegador no permite geolocalizaci贸n")
	...
	navigator.geolocation.getCurrentPosition(
		...
		(e)=>{	//On Error
			console.warn("No se pudo obtener geolocalizaci贸n")
...
function distanceGeo(lat1, lon1, lat2, lon2){
	...
	const d1 = lat1 * Math.PI/180; // 蠁, 位 in radians
...
// Esta funci贸n se ejecuta cuando volvemos de Flow con ?flowReturn=1&token=...
async function procesarFlow() {
    ...
    try {
        ...
        // Derivar plan en espa帽ol para updatePlan.php
        let planSp = "free";
        ...
        if ((upd && upd.success) || upd.status === "OK") {
            try{
                ...
                userData = u;
                const label = newPlanSp === 'basico' ? 'B谩sico' : (newPlanSp === 'premium' ? 'Premium' : 'Gratuito');
            ...
            if (typeof showAlertModal === 'function') {
                showAlertModal('Pago exitoso','Tu suscripci贸n fue actualizada.');
            } else {
                alert('Pago exitoso. Tu suscripci贸n fue actualizada.');
            }
            try{
