...
// by Michael Zaporozhets @ https://stackoverflow.com/a/11381730
...
async function processFrameContents(iFrame){
	...
	for(let input of iDocument.querySelectorAll("input[type=text]")){
	...
	for(let controller of iDocument.querySelectorAll(".fileUploadControl")){
		controller.addEventListener("click",()=>{
			iDocument.querySelector("#"+controller.dataset.input).click()
		})
	}
	
	// Handles elements that show images but arent don't have the img tag
	for(let pseudoImg of iDocument.querySelectorAll("[data-imgsrc]")){
	...
	for(let icon of iDocument.querySelectorAll("[data-icon]")){
		icon.style.backgroundImage="url(\"https://icons.getbootstrap.com/assets/icons/"+icon.dataset.icon+".svg\")";
	}
	...
	for(let grid of iDocument.querySelectorAll(".grid")){
		if(grid.dataset["columns"]){
			grid.style.gridTemplateColumns="repeat("+grid.dataset["columns"]+", 1fr)";
		}
	}
	for(let span of iDocument.querySelectorAll(".grid [colspan]")){
...
function loadModal(url){
	...
	fetch(url).then(r=>r.ok?r.text():"").then(t=>{
		...
		processFrameContents(modalBackdrop)
	})
}